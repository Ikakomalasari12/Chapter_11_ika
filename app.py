...
password = 'sparta'
cxn_str = f'mongodb+srv://test_ikakomalasari:{password}@test.nrkvr1l.mongodb.net/?retryWrites=true&w=majority'
...
def detail(keyword):
    ...
    if not definitions:
        ...
        'main',
        msg=f'Could not find word "{keyword}"'
    ))

    if not definitions:
        error_message = f'Could not find word "{keyword}"'
    ...
    if type(definitions[0]) is str:
        ...
        return redirect(url_for(
        'main',
        msg=f'Could not find word "{keyword}", did you mean one of these words:{suggestion}'
        ))
